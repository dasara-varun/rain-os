...
<p align="center">
  ...
  <a href="https://github.com/dasara-varun/rain-os/actions/workflows/release.yml"><img src="https://github.com/dasara-varun/rain-os/actions/workflows/release.yml/badge.svg?branch=v1.2.0" alt="Release Pipeline"></a>
...
Releases are cryptographically signed and tagged with semantic versioning (`v1.2.0`, etc.). Pushing a release tag automatically triggers the automated [Release Pipeline](.github/workflows/release.yml) which builds, validates, QEMU-tests, packages, and attaches all assets to the GitHub Release.

```bash
# Tag and trigger a release
git tag -a v1.2.0 -m "Rain OS Version 1.2.0 Production Release"
git push origin v1.2.0
```

### Released Assets & Manifests
Every official release provides the following downloadable artifacts:
1. **`rain-os-1.2.0-x86_64.iso`**: The full bootable live distribution ISO with Calamares graphical installer, hardware drivers, KDE Plasma 6, COSMIC profile, universal boot media auto-detection, and universal app ecosystem.
2. **`rain-os-1.2.0-source.tar.gz`**: Full, auditable source code repository tree for clean offline builds and open inspection.
3. **`rain-os-packages-1.2.0.tar.gz`**: Archive containing the compiled Rain OS local package repository (`rain.db.tar.zst`) and all pre-built `.pkg.tar.zst` packages.
...
Rain OS 1.2.0 features automated storage scan fallback (`archisosearchfilename`):
...
### Flashing to USB
- **Ventoy**: Simply copy `rain-os-1.2.0-x86_64.iso` to your Ventoy USB drive.
...
- **Linux (`dd`)**:
  ```bash
  sudo dd if=rain-os-1.2.0-x86_64.iso of=/dev/sdX bs=4M status=progress oflag=sync
  ```
